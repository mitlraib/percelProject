...
export default class WeddingSeatingBoardView {
  ...
  build(onSubmit: () => void): WeddingBoardRefs {
    ...
    const timerText = this.scene.add
      .text(
        panel.x - panel.width / 2 + (mobile ? 14 : 22),
        panelTop + (mobile ? 12 : 14),
        `זמן: ${this.durationSec}`,
        {
          fontFamily: "Arial",
  ...
  private createTables(
    ...
    const tableW = mobile ? 92 : 112;
    const tableH = mobile ? 62 : 78;

    const seatW = mobile ? 42 : 54;
    const seatH = mobile ? 20 : 26;

    const seatZoneW = mobile ? 50 : 62;
    const seatZoneH = mobile ? 28 : 34;
    ...
    for (let t = 0; t < this.canon.tablesCount; t++) {
      ...
      const seatPositions = [
        { x: tableX - (mobile ? 38 : 48), y: tableY - (mobile ? 5 : 8) },
        { x: tableX, y: tableY - (mobile ? 15 : 20) },
        { x: tableX + (mobile ? 38 : 48), y: tableY - (mobile ? 5 : 8) },
      ];
  ...
  private createButtons(
    ...
    const submitBtnBg = this.scene.add
      .rectangle(panel.x, buttonY, mobile ? 120 : 132, mobile ? 38 : 40, 0x6dbb75, 1)
